...
def _parse_list_arg(s, cast=float):
    ...
    s = s.strip()
    try:
        if s.startswith('['): vals = json.loads(s)
        else: vals = [x.strip() for x in s.split(',') if x.strip() != ""]
    except Exception as e: raise ValueError(f"Could not parse list argument: {s!r} ({e})")
    try: return [cast(v) for v in vals]
    except Exception as e: raise ValueError(f"Could not cast values in list argument {s!r} to {cast.__name__}: {e}")
...
def main(model_base_name, generation, num_subaccs, target, idle_leakage, heuristic, chips, estimates, powergating, preloading, solution, communication_time_scale, communication_energy_scale, idling_scale, power_scale, rt, dynamic, wakeup_time_scale, wakeup_cost_scale, early_termiantion, wb, lookup, topology, scale, max_subacc, chip_sizes, subaccs_per_chip, banks):
    ...
    G.add_nodes_from((node_id_mapping[node['id']], {
        ...
        'size': node['size'] or 0.0,
        'kv_size': node.get('kv_size', 0.0),
    }) for node in data['nodes'])
    G.add_edges_from((node_id_mapping[edge['source']], node_id_mapping[edge['destination']], 
    {'size': edge['size'], 
     'energy': edge.get('energy', 0.0) / (energy_factor * (generation + 1)), 
     'latency': edge.get('latency', 0.0) / (latency_factor * (generation + 1))}) 
    ...
    if max_subacc == 0: max_subacc = num_subaccs * num_chips
    subaccs_per_chip = _parse_list_arg(subaccs_per_chip, cast=int) if subaccs_per_chip else None
    chip_sizes   = _parse_list_arg(chip_sizes, cast=float) if chip_sizes else None
    if subaccs_per_chip is None: subaccs_per_chip = [num_subaccs] * num_chips
    if chip_sizes is None: chip_sizes = [p / float(sum(subaccs_per_chip)) for p in subaccs_per_chip]
    
    import hashlib, base64
    tag = base64.urlsafe_b64encode(hashlib.blake2b(json.dumps([args.chip_sizes, args.subaccs_per_chip], separators=(",", ":")).encode(), digest_size=9).digest()).decode().rstrip("=")
    problem = Problem(m=model_name, c=num_chips, p=num_subaccs, e=estimates, l=idle_leakage, id=idling_scale, pg=powergating,
                    ...
                    po=power_scale,  t=target, tp=topology, sc=scale, cs=tag, x='multi-chiplet')
    ...
    # dream_bound = dream_bound[target] if dream_bound else None
    ...
    if wb:
        ...
        wandb.log({
            ...
            "EDCP": partitioning['totalEDCP']*total_latency*total_energy if target == "EDCP" else None,
            ...
            "Chip Cost": partitioning['totalCost'] if target == "EDCP" else None,
    ...
    print(f"\n    - EDCP: {partitioning['totalEDCP']*total_energy*total_latency if target == 'EDCP' else 'N/A'}")
    ...
    print(f"    - Chip Cost: {partitioning['totalCost'] if target == 'EDCP' else 'N/A'}")
    ...
    if target == "MaxloadPipeline" or target == "EPT" or pipeline: 
        ...
        print(f"{partitioning['totalEDCP']*total_energy*total_latency}, {partitioning['totalCost']}, {partitioning['totalEDP']*total_latency*total_energy}")
    else:
        print(f"{partitioning['totalEDP']*total_energy*total_latency}, {partitioning['totalLatency']*total_latency}, {partitioning['totalEDP']*total_energy*total_latency}")
...
if __name__ == '__main__':
    ...
    parser.add_argument("-g", "--generation", type=int, help="Generation number to scale the model.")
    ...
    parser.add_argument("-cs", "--chip_sizes", type=str, default="", help="Per-chip size fractions that sum to 1. JSON or comma-separated.")
    parser.add_argument("-ppc", "--subaccs_per_chip", type=str, default="", help="Per-chip integer Subaccs that sum to total Subaccs. JSON or comma-separated.")
    parser.add_argument("-bk", "--banks", type=int, default=20, help="Number of banks per chip (default: 20)")
    args = parser.parse_args()

    # Execute main partitioning function with specified parameters
    main(args.model, args.generation, args.subAccs, args.target, args.leakage, args.heuristic, args.chips, args.estimates, args.powergating, args.preloading, args.solution, args.communication_time_scale, args.communication_energy_scale, args.idling_scale, args.power_scale, args.runtime, args.dynamic, args.wakeup_time_scale, args.wakeup_cost_scale, args.early_termination, args.wandb, args.lookup, args.topology, args.scale, args.max_subacc, args.chip_sizes, args.subaccs_per_chip, args.banks)
